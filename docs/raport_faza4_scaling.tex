...
\definecolor{fp8pccolor}{HTML}{56B4E9}
...
\begin{itemize}
  ...
  \item \textbf{FP8-pt} — stocare \texttt{float8\_e4m3fn} per-tensor,
    dequantizare la fiecare forward pass (clasa \texttt{FP8Linear});
  \item \textbf{FP8-pc} — stocare \texttt{float8\_e4m3fn} per-channel.
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrr}
    ...
    \midrule
    \textcolor{fp32color}{\textbf{FP32}}    & 75.454\% & ---                   & 21.81 & 28.4 \\
    \textcolor{fp16color}{\textbf{FP16}}    & 75.458\% & \dpos{+0.004}         & 10.91 & \phantom{0}9.0 \\
    \textcolor{int8ptcolor}{\textbf{INT8-pt}} & 75.128\% & \dneg{$-$0.326}    & \phantom{0}6.62 & 30.0 \\
    \textcolor{int8pccolor}{\textbf{INT8-pc}} & 75.418\% & \dneg{$-$0.036}    & \phantom{0}6.70 & 29.3 \\
    \textcolor{fp8ptcolor}{\textbf{FP8-pt}}   & 75.258\% & \dneg{$-$0.196}   & \phantom{0}6.62 & 30.1 \\
    \textcolor{fp8pccolor}{\textbf{FP8-pc}}   & 75.352\% & \dneg{$-$0.102}   & \phantom{0}6.70 & 29.5 \\
    \bottomrule
  \end{tabular}
  \label{tab:tiny}
\end{table}
...
FP16 obține un speedup de $\mathbf{3.15\times}$ față de FP32
(vs.\ $1.22\times$ pe M4~MPS), confirmând că A100 utilizează eficient
tensor cores. INT8-pc reduce degradarea de la $-0.326$~pp la $-0.036$~pp.
FP8-pt ($-0.196$~pp) este mai bun decât INT8-pt ($-0.326$~pp) dar mai slab
decât INT8-pc; FP8-pc ($-0.102$~pp) se situează între cele două INT8.
Latența FP8 este similară cu INT8 --- ambele dequantizează la FP32
înainte de matmul, deci nu există kernel nativ mai rapid.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrr}
    ...
    \textbf{Memorie (MB)} & \textbf{Latență (ms/batch)} \\
    \midrule
    \textcolor{fp32color}{\textbf{FP32}}    & 81.382\% & ---                  & 84.12  & 72.9 \\
    \textcolor{fp16color}{\textbf{FP16}}    & 81.394\% & \dpos{+0.012}        & 42.06  & 15.4 \\
    \textcolor{int8ptcolor}{\textbf{INT8-pt}} & 81.296\% & \dneg{$-$0.086}   & 23.37  & 73.8 \\
    \textcolor{int8pccolor}{\textbf{INT8-pc}} & 81.396\% & \dpos{+0.014}     & 23.53  & 73.6 \\
    \textcolor{fp8ptcolor}{\textbf{FP8-pt}}   & 81.374\% & \dneg{$-$0.008}  & 23.37  & 73.3 \\
    \textcolor{fp8pccolor}{\textbf{FP8-pc}}   & 81.306\% & \dneg{$-$0.076}  & 23.53  & 73.3 \\
    \bottomrule
  \end{tabular}
  \label{tab:small}
\end{table}
...
FP16 atinge $\mathbf{4.74\times}$ speedup. INT8-pc depășește FP32 cu
$+0.014$~pp. FP8-pt ($-0.008$~pp) este remarcabil de precis pe ViT-Small
--- mai bun decât INT8-pt și chiar mai bun decât FP8-pc ($-0.076$~pp).
Acesta este singurul model din studiu unde FP8 per-tensor surclasează FP8
per-channel, sugerând că distribuția ponderilor ViT-Small se potrivește
bine cu grila logaritmică a FP8 fără a necesita scale individuale.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrr}
    ...
    \textbf{Memorie (MB)} & \textbf{Latență (ms/batch)} \\
    \midrule
    \textcolor{fp32color}{\textbf{FP32}}    & 85.102\% & ---                  & 330.23 & 128.6 \\
    \textcolor{fp16color}{\textbf{FP16}}    & 85.110\% & \dpos{+0.008}        & 165.12 & \phantom{0}18.3 \\
    \textcolor{int8ptcolor}{\textbf{INT8-pt}} & 85.080\% & \dneg{$-$0.022}   & \phantom{0}87.23 & 129.8 \\
    \textcolor{int8pccolor}{\textbf{INT8-pc}} & 85.100\% & \dneg{$-$0.002}   & \phantom{0}87.55 & 130.0 \\
    \textcolor{fp8ptcolor}{\textbf{FP8-pt}}   & 85.046\% & \dneg{$-$0.056}  & \phantom{0}87.23 & 129.9 \\
    \textcolor{fp8pccolor}{\textbf{FP8-pc}}   & 85.094\% & \dneg{$-$0.008}  & \phantom{0}87.55 & 130.2 \\
    \bottomrule
  \end{tabular}
  \label{tab:base}
\end{table}
...
FP16 atinge $\mathbf{7.03\times}$ speedup --- cel mai mare speedup din
întreg studiul. INT8-pc pierde doar $-0.002$~pp, practic identic cu FP32.
FP8-pc ($-0.008$~pp) se comportă excelent, aproape la nivelul INT8-pc.
FP8-pt ($-0.056$~pp) este mai slab comparativ, dar rămâne mult mai bun
decât INT8-pt de la ViT-Tiny. La modele mari, FP8-pc revine la pattern-ul
normal (superior per-tensor), spre deosebire de ViT-Small.
...
pentru o comparație directă între cele trei arhitecturi și șase formate.

\begin{table}[H]
  \centering
  \caption{Tabel comparativ cross-model --- Faza~4 (NVIDIA A100, 16 mai 2026)}
  \resizebox{\linewidth}{!}{%
  \begin{tabular}{llrr>{\raggedleft\arraybackslash}p{1.8cm}>{\raggedleft\arraybackslash}p{2.2cm}}
    ...
    \textbf{Memorie (MB)} & \textbf{Latență (ms/batch)} \\
    \midrule
    \multirow{6}{*}{\textbf{ViT-Tiny} (5.7M)}
      & \textcolor{fp32color}{FP32}      & 75.454\% & ---             & 21.81 &  28.4 \\
      & \textcolor{fp16color}{FP16}      & 75.458\% & \dpos{+0.004}   & 10.91 &   9.0 \\
      & \textcolor{int8ptcolor}{INT8-pt} & 75.128\% & \dneg{$-$0.326} &  6.62 &  30.0 \\
      & \textcolor{int8pccolor}{INT8-pc} & 75.418\% & \dneg{$-$0.036} &  6.70 &  29.3 \\
      & \textcolor{fp8ptcolor}{FP8-pt}   & 75.258\% & \dneg{$-$0.196} &  6.62 &  30.1 \\
      & \textcolor{fp8pccolor}{FP8-pc}   & 75.352\% & \dneg{$-$0.102} &  6.70 &  29.5 \\
    \midrule
    \multirow{6}{*}{\textbf{ViT-Small} (22M)}
      & \textcolor{fp32color}{FP32}      & 81.382\% & ---             & 84.12 &  72.9 \\
      & \textcolor{fp16color}{FP16}      & 81.394\% & \dpos{+0.012}   & 42.06 &  15.4 \\
      & \textcolor{int8ptcolor}{INT8-pt} & 81.296\% & \dneg{$-$0.086} & 23.37 &  73.8 \\
      & \textcolor{int8pccolor}{INT8-pc} & 81.396\% & \dpos{+0.014}   & 23.53 &  73.6 \\
      & \textcolor{fp8ptcolor}{FP8-pt}   & 81.374\% & \dneg{$-$0.008} & 23.37 &  73.3 \\
      & \textcolor{fp8pccolor}{FP8-pc}   & 81.306\% & \dneg{$-$0.076} & 23.53 &  73.3 \\
    \midrule
    \multirow{6}{*}{\textbf{ViT-Base} (86M)}
      & \textcolor{fp32color}{FP32}      & 85.102\% & ---             & 330.23 & 128.6 \\
      & \textcolor{fp16color}{FP16}      & 85.110\% & \dpos{+0.008}   & 165.12 &  18.3 \\
      & \textcolor{int8ptcolor}{INT8-pt} & 85.080\% & \dneg{$-$0.022} &  87.23 & 129.8 \\
      & \textcolor{int8pccolor}{INT8-pc} & 85.100\% & \dneg{$-$0.002} &  87.55 & 130.0 \\
      & \textcolor{fp8ptcolor}{FP8-pt}   & 85.046\% & \dneg{$-$0.056} &  87.23 & 129.9 \\
      & \textcolor{fp8pccolor}{FP8-pc}   & 85.094\% & \dneg{$-$0.008} &  87.55 & 130.2 \\
    \bottomrule
  \end{tabular}}%
...
\begin{figure}[H]
  ...
  \caption{Acuratețe Top-1 (\%) pentru toate cele 3 modele și formatele
    FP32/FP16/INT8-pt/INT8-pc, evaluare pe NVIDIA~A100.}
...
% ===========================================================================
\section{Evaluare FP8 Native (float8\_e4m3fn)}
% ===========================================================================

FP8 E4M3FN a fost evaluat pe toate cele trei modele folosind clasa
\texttt{FP8Linear} (stocare reală \texttt{float8\_e4m3fn}, dequantizare
la fiecare forward pass, skip LayerNorm și head). Scriptul
\texttt{evaluate\_fp8\_native.py} a rulat pe același A100 pe 16 mai 2026
imediat după evaluarea INT8.

\subsection{Rezultate FP8 cross-model}

\begin{table}[H]
  \centering
  \caption{Comparație FP8 vs INT8 per-tensor și per-channel (NVIDIA A100)}
  \small
  \begin{tabular}{llrrr}
    \toprule
    \textbf{Model} & \textbf{Format} & \textbf{Accuracy} &
    \textbf{$\Delta$ FP32 (pp)} & \textbf{Memorie (MB)} \\
    \midrule
    \multirow{4}{*}{ViT-Tiny}
      & \textcolor{int8ptcolor}{INT8-pt} & 75.128\% & \dneg{$-$0.326} & 6.62 \\
      & \textcolor{fp8ptcolor}{FP8-pt}   & 75.258\% & \dneg{$-$0.196} & 6.62 \\
      & \textcolor{int8pccolor}{INT8-pc} & 75.418\% & \dneg{$-$0.036} & 6.70 \\
      & \textcolor{fp8pccolor}{FP8-pc}   & 75.352\% & \dneg{$-$0.102} & 6.70 \\
    \midrule
    \multirow{4}{*}{ViT-Small}
      & \textcolor{int8ptcolor}{INT8-pt} & 81.296\% & \dneg{$-$0.086} & 23.37 \\
      & \textcolor{fp8ptcolor}{FP8-pt}   & 81.374\% & \dneg{$-$0.008} & 23.37 \\
      & \textcolor{int8pccolor}{INT8-pc} & 81.396\% & \dpos{+0.014}   & 23.53 \\
      & \textcolor{fp8pccolor}{FP8-pc}   & 81.306\% & \dneg{$-$0.076} & 23.53 \\
    \midrule
    \multirow{4}{*}{ViT-Base}
      & \textcolor{int8ptcolor}{INT8-pt} & 85.080\% & \dneg{$-$0.022} & 87.23 \\
      & \textcolor{fp8ptcolor}{FP8-pt}   & 85.046\% & \dneg{$-$0.056} & 87.23 \\
      & \textcolor{int8pccolor}{INT8-pc} & 85.100\% & \dneg{$-$0.002} & 87.55 \\
      & \textcolor{fp8pccolor}{FP8-pc}   & 85.094\% & \dneg{$-$0.008} & 87.55 \\
    \bottomrule
  \end{tabular}
  \label{tab:fp8cross}
\end{table}

\subsection{Observații FP8}

\textbf{Memoria identică cu INT8.} FP8 și INT8 stochează 1 byte/parametru,
deci reducerea de memorie față de FP32 este aceeași ($\approx 3.3$--$3.8\times$).

\textbf{Latența identică cu INT8.} Implementarea dequantizează la FP32
înainte de \texttt{F.linear}, deci nu există kernel FP8 GEMM nativ.
Pe A100, latența FP8 este practic identică cu INT8.

\textbf{FP8-pt bate INT8-pt pe Tiny și Small.} Distribuția logaritmică
a FP8 gestionează mai bine outlierii față de scalarea uniformă a INT8
per-tensor. Câștigul: $+0.130$~pp pe Tiny, $+0.078$~pp pe Small.
Pe Base diferența se inversează ($-0.034$~pp), deoarece modelele mari
au distribuții mai uniforme unde INT8 uniform funcționează bine.

\textbf{Anomalia ViT-Small: FP8-pt $>$ FP8-pc.} Pe ViT-Tiny și ViT-Base,
FP8-pc este mai precis decât FP8-pt (pattern normal). Pe ViT-Small,
FP8-pt ($-0.008$~pp) surclasează FP8-pc ($-0.076$~pp) --- cel mai bun
rezultat FP8 din întreg studiul. O posibilă explicație: la ViT-Small
(embed\_dim=384), ponderile de pe fiecare canal au variații relativ mari
între canale, iar scale-urile per-channel FP8 introduc mai mult zgomot
de cuantizare decât un scalar global bine ales.

\textbf{FP8 pe hardware nativ (H100).} Pe GPU-uri Hopper (compute
capability~$\geq 9.0$), kernelele FP8 GEMM sunt disponibile nativ.
Latența FP8 ar putea fi comparabilă cu FP16. Rezultatele de acuratețe
sunt valide pe orice hardware; latența raportată reflectă doar
implementarea software actuală.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrr}
    ...
    \midrule
    ViT-Tiny  (5.7M) &  28.4 ms & \phantom{0}9.0 ms & $\mathbf{3.15\times}$ & $1.22\times$ \\
    ViT-Small (22M)  &  72.9 ms & 15.4 ms & $\mathbf{4.74\times}$ & --- \\
    ViT-Base  (86M)  & 128.6 ms & 18.3 ms & $\mathbf{7.03\times}$ & --- \\
    \bottomrule
  \end{tabular}
...
Pe A100, același ViT-Tiny obține $3.15\times$, iar la ViT-Base ajunge
la $7.03\times$. Explicația constă în natura calculului:
\begin{itemize}
...
\begin{figure}[H]
  ...
  \caption{Speedup FP16 față de FP32 pe NVIDIA A100 în funcție de
    ...
    de la $3.15\times$ (Tiny) la $7.03\times$ (Base).}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrr}
    ...
    \midrule
    FP32    & 2.578 ms & 0.222 ms & $\mathbf{11.6\times}$ \\
    FP16    & 2.111 ms & 0.070 ms & $\mathbf{30.2\times}$ \\
    INT8-pt & 4.299 ms & 0.235 ms & $\mathbf{18.3\times}$ \\
    INT8-pc & 4.545 ms & 0.229 ms & $\mathbf{19.9\times}$ \\
    \bottomrule
  \end{tabular}
  \label{tab:hardware}
\end{table}
...
\begin{enumerate}
  \item \textbf{FP32}: A100 este de $11.6\times$ mai rapid --- beneficiaza
  ...
  \item \textbf{INT8}: pe M4, INT8 este \emph{mai lent} decât FP32 (overhead
    ...
    Speedup-ul A100/M4 ajunge la $18.3$--$19.9\times$ deoarece startul
...
$3.15\times$ (Tiny) până la $7.03\times$ (Base).
...
valorile reflectă intervalul observat pe NVIDIA A100 (Tiny--Base):
{\small\[
  \underbrace{\text{INT8-mlp-pc}}_{\text{+0.030 pp}} \geq
  \underbrace{\text{FP16}}_{\approx 0} \approx
  \underbrace{\text{INT8-pc}}_{-0.002\text{--}-0.036} \approx
  \underbrace{\text{FP8-pc}}_{-0.008\text{--}-0.102} >
  \underbrace{\text{FP8-pt}}_{-0.008\text{--}-0.196} >
...
\begin{table}[H]
  ...
  \begin{tabular}{lp{9.5cm}}
    ...
    \midrule
    Script INT8/FP16 & \texttt{scripts/phase4/evaluate\_model.py} \\
    Script FP8 & \texttt{scripts/phase4/evaluate\_fp8\_native.py} \\
    Script comparație & \texttt{scripts/phase4/compare\_cross\_model.py} \\
    Rezultate INT8 & \texttt{results/Phase4/<model>/metrics/results.json} \\
    Rezultate FP8 & \texttt{results/Phase4/<model>/fp8\_native/metrics/fp8\_results.json} \\
    \bottomrule
  \end{tabular}
  \label{tab:config}
\end{table}
