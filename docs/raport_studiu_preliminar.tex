...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    Inițializare ponderi & Aleatorie (\texttt{pretrained=False}) \\
    Dataset antrenare    & CIFAR-10 \\
    Device               & Apple M4 MPS \\
    PyTorch              & 2.9.1 \\
    \bottomrule
  \end{tabular}
...
\begin{itemize}
  ...
  \item Timpii de antrenare sunt comparabili ($\pm 4$\%)
    --- variațiile pot fi cauzate de condiții de sistem, nu de precizia numerică
\end{itemize}
...
\begin{figure}[H]
  ...
  \caption{Timpul total de antrenare. Toate experimentele durează
    $\approx 7$--7.5~ore pe Apple M4 MPS, cu variații de $\pm 3$\% între
    configurații.}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrr}
    ...
    FP8 per-channel   & 83.42\% & $+0.01$~pp  & 0.5576 \\
    \bottomrule
  \end{tabular}
